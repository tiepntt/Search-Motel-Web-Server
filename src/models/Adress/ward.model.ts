...
import { Ward } from "../../entity/adress/Ward";
import districtRouter from "../../routers/Adress/district.router";
...
const create = async (input: WardInputDto) => {
  if (!input || !input.code || !input.districtId || !input.name)
    return HandelStatus(400);
  let wardRepo = getRepository(Ward);
  let districtRepo = getRepository(District);
  let district = await districtRepo.findOne(input.districtId);
